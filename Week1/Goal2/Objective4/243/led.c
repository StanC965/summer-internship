...
void BlinkFast_LED(volatile unsigned char *port_reg, unsigned char pin_number)
{
    toggle_pin(port_reg, pin_number);
    for (volatile unsigned long i = 0; i < 60000; i++);
}

void BlinkSlow_LED(volatile unsigned char *port_reg, unsigned char pin_number)
{
    toggle_pin(port_reg, pin_number);
    for (volatile unsigned long i = 0; i < 175000; i++);
}
