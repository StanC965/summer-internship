...
/** \fn     unsigned char read_pin(volatile unsigned char *pin_reg, unsigned char pin_number)
    \brief  Reads the logical value of a specific pin from a PINx register.
    \return 1 if the pin is HIGH, 0 if the pin is LOW.
*/
extern unsigned char read_pin(volatile unsigned char *pin_reg, unsigned char pin_number);
